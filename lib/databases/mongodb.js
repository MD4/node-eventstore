  ...
  connect: function (callback) {
    ...
    client.connect(connectionUrl, options.options, function(err, db) {
      ...
      function finish (err) {
        ...
        async.parallel([
          function(cb) {
            self.events.ensureIndex({ aggregateId: 1, streamRevision: 1 }, cb);
          },
          function(cb) {
            self.events.ensureIndex({ commitStamp: 1 }, cb);
          },
          function(cb) {
            self.events.ensureIndex({ dispatched: 1 }, { sparse: true }, cb);
          },
          function(cb) {
            self.events.ensureIndex({ commitStamp: 1, streamRevision: 1, commitSequence: 1 }, cb);
          },
          function(cb) {
            self.snapshots.ensureIndex({ aggregateId: 1, revision: -1 }, cb);
          },
          function(cb) {
            self.transactions.ensureIndex({ aggregateId: 1, 'events.streamRevision': 1 }, cb);
          }
        ], function(err) {
          if (err) { debug(err); }
